...
class ChatViewModel(
    ...
    private fun translateText(text: String, onResult: (String) -> Unit) {
        ...
        translator?.translate(text)
            ...
            ?.addOnFailureListener { exception ->
                ...
                onResult(text) // Fallback to original text
            }
    }
    ...
    fun sendMessage(content: String) {
        ...
        if (_isTranslationEnabled.value) {
            translateText(content) { translatedContent ->
                 processAndSendMessage(translatedContent)
            }
        } else {
             processAndSendMessage(content)
        }
    }

    private fun processAndSendMessage(content: String) {
        if (content.isEmpty()) return
