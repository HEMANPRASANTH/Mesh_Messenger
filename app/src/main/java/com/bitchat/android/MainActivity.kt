...
import com.bitchat.android.onboarding.WelcomeScreen
import com.bitchat.android.ui.theme.ThemeControl
...
class MainActivity : OrientationAwareActivity() {
    ...
    override fun attachBaseContext(newBase: android.content.Context) {
        super.attachBaseContext(com.bitchat.android.util.LocaleManager.updateBaseContext(newBase))
        // com.google.android.play.core.splitcompat.SplitCompat.installActivity(this) // Removed: dependency missing and not strictly needed for debug
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            // Theme State Management (Hoisted)
            // Default to system setting, but allow overriding via SettingsScreen
            // Using a CompositionLocal or ViewModel would be cleaner in a larger app, 
            // but for this task, passing the state down or using a shared preference observer is sufficient.
            // checking simple shared prefs for theme
            val prefs = getSharedPreferences("app_prefs", MODE_PRIVATE)
            val isSystemDark = isSystemInDarkTheme()
            val savedThemeMode = remember { mutableIntStateOf(prefs.getInt("theme_mode", 0)) } // 0=System, 1=Light, 2=Dark
            
            val useDarkTheme = when (savedThemeMode.intValue) {
                1 -> false
                2 -> true
                else -> isSystemDark
            }
            ...
            val onThemeChanged: (Int) -> Unit = { mode ->
                savedThemeMode.intValue = mode
                prefs.edit().putInt("theme_mode", mode).apply()
            }

            BitchatTheme(darkTheme = useDarkTheme) {
                Scaffold(
                    modifier = Modifier.fillMaxSize(),
                    containerColor = MaterialTheme.colorScheme.background,
                    contentWindowInsets = WindowInsets(0.dp, 0.dp, 0.dp, 0.dp) // Fix double padding: Let ChatScreen handle insets
                ) { innerPadding ->
                    // Pass savedThemeMode/onThemeChanged to screens later via CompositionLocal or explicit params
                    // extending OnboardingFlowScreen to accept navigation for now?
                    // Actually, we will inject this via a CompositionLocal for easier access in nested screens
                    
                    CompositionLocalProvider(LocalThemeControl provides ThemeControl(savedThemeMode.intValue, onThemeChanged)) {
    ...
    enum class AppScreen {
        ONBOARDING, CHAT, ACCOUNT, SETTINGS, FEATURES
    }
    ...
    private fun OnboardingFlowScreen(modifier: Modifier = Modifier) {
        ...
        // Navigation State for Post-Onboarding
        ...
        var isSignedUp by remember { mutableStateOf(prefs.getBoolean("is_signed_up", false)) }
        
        // Initial check: if onboarding is complete, go to chat ONLY if signed up
        LaunchedEffect(onboardingState, isSignedUp) {
            if (onboardingState == OnboardingState.COMPLETE && currentScreen == AppScreen.ONBOARDING) {
                if (isSignedUp) {
        ...
        // Use a persistent nickname state for the session (real app would persist to prefs/db)
        // Initialize from ViewModel cache or prefs if possible, defaulting to "User_XXXX"
        var nickname by remember { mutableStateOf(chatViewModel.nickname.value.takeIf { it.isNotEmpty() } ?: "User_" + (1000..9999).random()) }
        ...
        // Main Navigation Host (Simple conditional switching)
        Box(modifier = modifier) {
            when (currentScreen) {
                AppScreen.CHAT, AppScreen.ACCOUNT -> {
                    // Drawer State
                    ...
                    ModalNavigationDrawer(
                        ...
                        drawerContent = {
                            ModalDrawerSheet(
                                ...
                                // The drawer content itself can be a NavHost or a simple Column with navigation items
                                // For this change, we'll assume a simple Column with direct composable calls
                                // and update the existing AccountContent to include onOpenFeatures.
                                // Note: The provided snippet for the change was syntactically incorrect for a ModalDrawerSheet.
                                // I'm interpreting the intent to update the existing AccountContent and add a FeaturesScreen route.
                                
                                // Original AccountContent was here. Let's update it.
                                com.bitchat.android.ui.AccountContent(
                                    currentNickname = nickname,
                                    onNicknameChanged = { newName ->
                                        nickname = newName
                                        chatViewModel.setNickname(newName)
                                    },
                                    onOpenFeatures = {
                        ...
                        if (currentScreen == AppScreen.CHAT) {
                            // Chat Screen with Chat-Specific Back Logic
                            ...
                            onBackPressedDispatcher.addCallback(context as androidx.activity.ComponentActivity, backCallback)
                            
                            ChatScreen(
                                viewModel = chatViewModel,
                                currentUserNickname = nickname, // Pass strictly for display if needed, but VM is source of truth
                                onAvatarClick = { 
                                    scope.launch { drawerState.open() }
                                }
                            )
                        } else {
                            // Account Screen Logic (if accessed via other means, though Drawer covers most info)
                            // Keeping it for potential deep link or other nav, but Drawer is primary now.
                            com.bitchat.android.ui.AccountScreen(
                                currentNickname = nickname,
                                onNicknameChanged = { newName -> 
                                    chatViewModel.setNickname(newName) // SYNC: Update ViewModel immediately
                                },
                ...
                AppScreen.SETTINGS -> {
                    com.bitchat.android.ui.SettingsScreen(
                        onBack = { currentScreen = AppScreen.ACCOUNT }
                    )
                }
                AppScreen.FEATURES -> {
                    com.bitchat.android.ui.FeaturesScreen(
                        onBack = { currentScreen = AppScreen.ACCOUNT }
                    )
                }
                AppScreen.ONBOARDING -> {
                    // SHOW WELCOME SCREEN FIRST if not signed up, regardless of onboarding state
                    // This creates the "Auth Page" experience the user requested
                    // SHOW WELCOME SCREEN FIRST if not signed up, regardless of onboarding state
                    // This creates the "Auth Page" experience the user requested
                    if (!isSignedUp) { 
                         WelcomeScreen(
                             onGetStarted = {
                                 // Mark as signed up and proceed
                                 isSignedUp = true
                                 prefs.edit().putBoolean("is_signed_up", true).apply()
                                 
                                 // If already onboarded (e.g. permission granted previously), go to chat
                                 if (onboardingState == OnboardingState.COMPLETE) {
                                     currentScreen = AppScreen.CHAT
                                 } else {
                                     // Otherwise ensure we are checking/initializing
  ...
  // Standard Onboarding Flow
            ...
            OnboardingState.CHECKING, OnboardingState.INITIALIZING, OnboardingState.COMPLETE -> {
                 // Nothing to render here as we switch to AppScreen.CHAT
                 // But we keep this state valid for the state machine
                 // Just show a placeholder or empty box until the LaunchedEffect switches the screen
